...
impl<'s> Iterator for BitcoinScriptScan<'s> {
    ...
    fn next(&mut self) -> Option<<Self as Iterator>::Item> {
        while let Some((_, envelope)) = self.dag.next() {
            if let Some(transaction) = match Payload::deserialize(envelope.payload()).expect("can not unwrap data envelope") {
...
mod test {
    ...
    fn block_test() {
        ...
        let mut db = BitcoinAdapter::new(Transient::new_db("first", 1).unwrap());
